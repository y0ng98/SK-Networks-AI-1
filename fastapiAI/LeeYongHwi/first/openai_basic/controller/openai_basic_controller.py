...
async def talkWithOpenAI(openAITalkRequestForm: OpenAITalkRequestForm,
    ...
    print(f"controller -> letsTalk(): openAITalkRequestForm: {openAITalkRequestForm}")
...
@openAIBasicRouter.post("/openai-with-langchain")
async def chatWithLangChain(
        openAITalkRequestForm: OpenAITalkRequestForm,
        openAIBasicService: OpenAIBasicServiceImpl =
        Depends(injectOpenAIBasicService)):

    print(f"controller -> chatWithLangChain(): "
          f"openAITalkRequestForm: {openAITalkRequestForm}")

    chatResponse = await openAIBasicService.chatWithLangChain(openAITalkRequestForm.userSendMessage)

    return JSONResponse(content={"result": chatResponse},status_code=status.HTTP_200_OK)